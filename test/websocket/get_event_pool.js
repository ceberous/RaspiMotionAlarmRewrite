...
function get_log() {
	const key = "sleep.raspi.python.event_pools." + get_eastern_time_key_suffix();
	console.log( key );
...
ws.on( "message" , ( data )=> {
	...
	if ( data.message === "new_event_pools" ) {
		data = data.data;
		...
		for ( let i = 0; i < decrypted_messages.length; ++i ) {
			...
			console.log( decrypted_messages[ i ].event_pool );
			console.log( decrypted_messages[ i ].extra_alert_pool );
		}
	}
	process.exit( 1 );
});